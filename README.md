...
5. **要不要动**：三选一——改变 / 待验证 / 未改变。一句话说清影响对哪个公司或赛道的判断，团队扫一眼就知道要不要细看

每一步都依赖前一步：实验不靠谱 → 量级不成立 → 格局没变 → 影响不存在 → 不用动。链条断在哪里，后面就不用看了。

论文里的公式和术语对非技术背景的同事不太友好，所以全程用类比来拆解技术逻辑和实验设计——重点讲清楚每个实验在验证什么机制、结果意味着什么，把生涩的术语转化成可以直接支撑投资判断的语言。
